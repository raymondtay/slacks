...
object JsonCodecLens {
  ...
  def getFileThumbVideoValue : Reader[io.circe.Json, Option[String]] =
    Reader{ (json: io.circe.Json) ⇒ root.file.thumb_video.string.getOption(json)}
  ...
  def isTeamFieldPresent : Reader[io.circe.Json, Boolean] =
    Reader{ (json: io.circe.Json) ⇒ root.team.obj.getOption(json).fold(false)(_ ⇒ true) }
